...
const AsciiArt = () => {
  ...
  const replacedLines = asciiLines.map(line => line.replace(regex, replacements[0]));
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const regex = new RegExp(searchString, 'g');
      const replacedLines = asciiLines.map(line => line.replace(regex, replacements[replacementIndex]));
